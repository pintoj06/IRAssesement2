...
class newRobotSystem:
    def __init__(self, ur3: 'UR3e', gp4: 'newGP4', rebel: 'ReBeL', cenTop, stop_event, restart_event: 'threading.Event', dummyMoveThread, ttList: 'list', topList: 'list', env: 'swift.Swift', ee: 'list' = [], eeToolOffset: 'list' = [], specimen1LiquidList: 'list' = [], specimen2LiquidList: 'list' = []):
        ...
        self.specimen2LiquidList = specimen2LiquidList # List of specimen liquids 2
       ...
       # self.boolSpecimen1 = True
        ...
        greenAlarmFile = '/Users/harrymentis/Documents/SensorsAndControls/Assignment2/greenAlarm.dae' # FOR HARRY
        ...
        redAlarmFile = '/Users/harrymentis/Documents/SensorsAndControls/Assignment2/redAlarm.dae' # FOR HARRY
        ...
        self.dummyMoveThread = dummyMoveThread
        




    def simulation(self, specimen_plan: 'list'):
        self.greenAlarm.T = self.alarmhide # Hide green alarm
        self.redAlarm.T = self.alarmshow # Show red alarmf
        self.updateTestTubeList(specimen_plan)
        print("Specimen Plan: ", specimen_plan)

        self.testRRTPastObj()
        self.testRRTPastObjrebel()

        #REBEL COLLISON TEST
        #self.jtrajMoverebel([1.74, 0, 0,0,0,0]) # Move to starting position
        #self.jtrajMovegp4([-pi, -pi/2, pi/2, 0, 0, 0])

        self.fillTubes() # Fill the test tubes with sample
        #self.dummyMoveThread.start()
    ...
    def updateTestTubeList(self, specimen_plan: 'list'):
        """Update the test tube list to include which specimen it has"""
        for i in range (0, len(self.ttList)):
            self.ttList[i].specimen = specimen_plan[i]
    ...
    def fillTubes(self):
        ...
        if self.useRRT:
            for i in range(0, len(self.ttList)): # Repeat for each test tube
                if self.ttList[i].specimen == 1:
                    setlist = self.specimen1LiquidList
                    specimenLocation = self.specimen1LiquidList[0].startPos
                else:
                    setlist = self.specimen2LiquidList
                    specimenLocation = self.specimen2LiquidList[0].startPos
                #find traj 
                specimenLocation = setlist[i].startPos
                q1 = specimenLocation @ SE3.Ry(pi/2).A
                q2 = q1 @ SE3(-0.17,0,0).A
                q_goal= self.rebel.ik_LM(q2, q0 = [3.027, 1.101, 0.087, 0.105, -1.264, 0.0])[0]
                #action
                self.primRRTrebel(q_goal, [self.collisionDet.testRRtrebel, self.collisionDet.table]) # Go to the sample
                setlist[i].attachToRobot(self.rebel, self.env)                                                     # Slurp slurp
                ...
                self.primRRTrebel(q_goal, [self.collisionDet.testRRtrebel, self.collisionDet.table], setlist[i]) # 
                self.ttList[i].attachSpecimen(setlist[i])
                self.rmrcrebel(-0.2, 3, [setlist[i]], objOffset=[SE3(0,0,0).A])
                setlist[i].attachToTestTube(self.ttList[i].startPos, self.env)
                self.rmrcrebel(0.2, 3) 
            ...
            for i in range(0, len(self.ttList)): # Repeat for each test tube
                ...
                self.rmrcrebel(-0.11, 3) #move down to fill tube

                setlist[i].attachToRobot(self.rebel, self.env)
                self.rmrcrebel(0.21, 3, [setlist[i]]) #move down to fill tube
                q_goal = self.rebel.ik_LM(self.ttList[i].startPos @ self.ttList[i].offset @ troty(-pi/2) @ SE3(-0.2,0,0).A, q0 = [-1.303, 1.303, -0.410, 0.311, -0.583, 0.000])[0]
                self.jtrajMoverebel(q_goal, setlist[i])

                self.rmrcrebel(-0.2, 3, [setlist[i]], objOffset=[SE3(0,0,0).A]) #move down to fill tube
                self.ttList[i].attachSpecimen(setlist[i])
                setlist[i].attachToTestTube(self.ttList[i].startPos, self.env)
    ...
    def moveToCentrifuge(self):
            ...
            for i in range(len(self.ttList)): # Repeat the following for each test tube
                ...
                self.rmrcur3(0.11, 2, [self.topList[i], self.ttList[i], self.ttList[i].specimenFull], objOffset=[self.topList[i].ttoffset, self.ttList[i].offset,SE3(0,0,0.13).A]) # Move the end effector directly up to remove the test tube from the holder
                self.jtrajMoveur3(self.ur3.ik_LM(self.ttList[i].endPos @ self.ur3RMRCoffsetTwo @ trotx(pi))[0], moveObj = [self.topList[i], self.ttList[i], self.ttList[i].specimenFull], objOffset=[self.topList[i].ttoffset, self.ttList[i].offset, SE3(0,0,0.13).A]) # Move directly to the test tubes final position located in the centrifuge # Move directly down to place the test tube in the centrifuge
                self.rmrcur3(-0.1, 2, [self.topList[i], self.ttList[i],  self.ttList[i].specimenFull], objOffset=[self.topList[i].ttoffset, self.ttList[i].offset, SE3(0,0,0.13).A]) # Move directly down to place the test tube in the centrifuge
    ...
    def moveToRack(self):
            for i in range(len(self.ttList)): # Repeat the following for each test tube
                ...
                self.rmrcur3(0.1, 2, [self.topList[i], self.ttList[i], self.ttList[i].specimenFull], objOffset=[self.topList[i].ttoffset, self.ttList[i].offset, SE3(0,0,0.13).A]) # Move the end effector directly up to remove the test tube from the centrifuge
                self.jtrajMoveur3(self.ur3.ik_LM(self.ttList[i].startPos @ SE3(0,0,0.28).A @ trotx(pi) @ trotz(pi/2))[0], [self.topList[i], self.ttList[i], self.ttList[i].specimenFull], objOffset=[self.topList[i].ttoffset, self.ttList[i].offset, SE3(0,0,0.13).A]) # Move the test tube directly above the rack
                self.rmrcur3(-0.085, 2, moveObj = [self.topList[i], self.ttList[i] , self.ttList[i].specimenFull], objOffset=[self.topList[i].ttoffset, self.ttList[i].offset, SE3(0,0,0.13).A]) # Move the end effector directly down to place the test tube in the rack
                self.ur3EE.openGripper() # Open the gripper to release the test tube
            self.jtrajMoveur3([0,-pi/2,0,0,0,0]) # Move back to the starting position
    ...
    def testRRTPastObjrebel(self):
        self.collisionDet.testRRTrebel() # Add the object that the robot needs to avoid to the environment
        self.useRRT = True # Boolean used to declare the use of RRT in the movement functions
    ...
    def primRRTrebel(self, endq, myObj: 'list', moveObj: 'list' = None, objOffset: 'list' = None):
        collisions = []
        q1 = self.rebel.q
        q2 = endq
        q_waypoints = np.array([q1, q2])  # Start and end positions
        ...
        while is_collision_check:
            ...
            for i in range(start_waypoint, len(q_waypoints) - 1):
                self.checkGUIStop()
                q_matrix_join = self.collisionDet.interpolate_waypoints_radians(
                    [q_waypoints[i], q_waypoints[i + 1]], np.deg2rad(10)
                )

                if (not self.collisionDet.is_collision(self.rebel.dhRobot, q_matrix_join,
                                                    myObj[0].faces, myObj[0].vertices, myObj[0].face_normals,
                                                    collisions, return_once_found=True)
                    and not self.collisionDet.is_collision(self.rebel.dhRobot, q_matrix_join,
                                                        myObj[1].faces, myObj[1].vertices, myObj[1].face_normals,
                                                        collisions, return_once_found=True)):
                    q_matrix.extend(q_matrix_join)
                    for q in q_matrix_join:
                        self.checkGUIStop()
                        self.rebel.q = q
                        self.rebel.dhRobot.q = self.rebel.q
                ...
                # move mesh1 and update other meshes accordingly
                    ...
                    q_rand[4] = random.uniform(-123/180, 123/180) * np.pi
                    q_rand[5] = random.uniform(-455/180, 455/180) * np.pi
                    q_rand = q_rand.tolist()  # Convert to a 6-element list

                    while (self.collisionDet.is_collision(self.rebel.dhRobot, [q_rand],
                                                        myObj[0].faces, myObj[0].vertices, myObj[0].face_normals,
                                                        collisions, return_once_found=True)
                        and self.collisionDet.is_collision(self.rebel.dhRobot, [q_rand],
                                                            myObj[1].faces, myObj[1].vertices, myObj[1].face_normals,
                                                            collisions, return_once_found=True)):
                        self.checkGUIStop()
                        q_rand = np.zeros(6)
                        q_rand[0] = random.uniform(-17/18, 17/18) * np.pi
                        q2_min, q2_max = np.deg2rad(10),  np.deg2rad(120)   # shoulder elevation stays positive
                        q3_min, q3_max = np.deg2rad(-25), np.deg2rad(70)
                        q_rand[1] = random.uniform(q2_min, q2_max)      # constrained q2
                        q_rand[2] = random.uniform(q3_min, q3_max)
                        q_rand[3] = random.uniform(-20/18, 20/18) * np.pi
                        q_rand[4] = random.uniform(-123/180, 123/180) * np.pi
                        q_rand[5] = random.uniform(-455/180, 455/180) * np.pi
