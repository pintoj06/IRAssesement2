...
class JointControlUI:
    def __init__(self, botSystem: 'newRobotSystem', stop_event: threading.Event, restart_event: threading.Event):
        ...
        self.root.mainloop()
    def _collect_specimen_plan(self):
        """Return a list of six ints (1 or 2) based on the radio selections."""
        if not hasattr(self, "specimen_vars") or not self.specimen_vars:
            # Fallback if the Sim panel hasn't been built for some reason
            return [1, 1, 1, 1, 1, 1]
        # Convert "1"/"2" StringVar values to ints
        return [int(v.get()) for v in self.specimen_vars]
...
# Inside the for-loop for i in range(6):
    ...
    def _build_cartesian_panel(self, parent):
        ...
        ttk.Button(qa, text=" ", command=lambda: self._cart_set(x=0.0, y=0.0, keep="Z")).grid(row=0, column=0, sticky="ew", padx=(0, 6))
        ttk.Button(qa, text=" ", command=lambda: self._cart_set(z=0.20, keep="XY")).grid(row=0, column=1, sticky="ew", padx=(6, 0))
    ...
    def on_run(self):
        ...
        if self.inStart:
            ...
            # <-- NEW: grab the tube selections (1/2) before starting the thread
            specimen_plan = self._collect_specimen_plan()

            # Pass the plan into the motion thread
            self.run_thread = threading.Thread(
                target=self._run_motion,
                args=(specimen_plan,),   # <-- pass as arg
                daemon=True
            )
    ...
    def _run_motion(self, specimen_plan):
        # Example: RMRC-like stepping loop (runs in thread)
        print("SIMULATION STARTED")
        # <-- NEW: pass the list into your simulation
        self.robotSystem.simulation(specimen_plan)
